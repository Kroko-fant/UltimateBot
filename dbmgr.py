...
class DbMgr:
	...
	def get(self, guild):
		...
		return self.db_handles[str(guild)]
	...
	def upgrade_db(self, connection):
		...
		while os.path.isfile(f"db/db-patterns/schema_{user_version + 1}.sql"):
			with open(f"db/db-patterns/schema_{user_version + 1}.sql") as file:
