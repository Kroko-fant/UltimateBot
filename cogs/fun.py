...
class Fun(commands.Cog):
	...
	async def metafrage(self, ctx):
		...
		metafrageembed = discord.Embed(
			title="Metafrage",
			description=
			'Du hast gerade eine Metafrage gestellt. Eine Metafrage ist eine unnötige Frage über der HauptFrage. Dies'
			' ist zwar höflich, aber ziemlich umständlich, da man seine Frage dann zwei mal stellen muss. \n\nDie Frage'
			' "Kennt sich jemand mit x aus?" hilft leider auch nicht weiter. Die Anwesenden könnten eventuell bei dem'
			' Problem helfen, obwohl sie nicht von sich behaupten würden, mit dem Thema vertraut zu sein. Und auch wenn'
			' jemand mit dem erfragten Thema vertraut ist, bedeutet dies nicht, dass er/sie eine spezielle Frage zu'
			' diesem Thema beantworten kann – niemand ist allwissend.\n\nDeswegen, stelle bitte einfach deine Frage'
			' anstatt "Darf ich etwas fragen" oder "kennt sich jemand mit x und y aus?" zu schreiben. Selbst, wenn es'
			' eine komplizierte oder sehr spezielle Frage ist, die du mehrere Minuten lang formulieren müsstest, dann'
			' mach das bitte. Wir wissen sonst nicht, ob wir sie beantworten könnten. \nWeitere Infos findest du unter'
			' http://metafrage.de/')
